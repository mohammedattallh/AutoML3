       ...
       for col in categorical_cols:
        if df[col].isnull().sum() > 0:
            if df[col].dtype.name != 'int64' or 'float64': 
               ...
               option = st.selectbox(f"Handling missing values in '{col}'", ['None','Most Frequent', 'Additional Class'])
               if option == 'Most Frequent':
                    df[col].fillna(df[col].mode()[0], inplace=True)
               elif option == 'Additional Class':
                     add_class = st.text_input('Enter additional class of missing value')
                     df[col].fillna(add_class, inplace=True) 

       for col in numerical_cols:
          if df[col].isnull().sum() > 0:
            if df[col].dtype.name != 'category' or 'object' or 'bool':  

                option = st.selectbox(f"Handling missing values in '{col}'", ['None','Mean', 'Median', 'Mode'])
                if option == 'Mean':
...
if choice == 'EDA':
     ...
     col_drop = st.text_input('name column for drop')
     btn = st.button('Drop')
     if btn :
         if col_drop != '':
     ...
     st.info('handling missing value ')
     automate_preprocessing(df,categorical_cols,numerical_cols) 
     st.info("number null for columns ")
     st.write(df.isna().sum())

     option = st.radio(f" Choose type visualization to Data  ", ['None','Plot','Hieatmap', 'Scatter', 'Bar','Histogram','Violin plot','Box plot'])
     if option== 'Heatmap':
